 ...
 *
 * Usage: npx tsx audit/content/status.ts [subject] [--json]
 *
 * Also importable: `import { generateContentStatus } from "./content-status.js"`
 */
...
export type ContentStatusResult = {
  ...
  topicsBelow50: number;
  topicsWithIssues: number;
  simDataAvailable: boolean;
  calibrationIssueCount: number;
  topics: TopicHealth[];
};
...
export function generateContentStatus(subject: string): ContentStatusResult | null {
  ...
  const contentQualityPath = join(process.cwd(), "audit", "reports", "content-quality.md");
  ...
  const totalProblems = Array.from(problemsByTopic.values()).reduce((s, ps) => s + ps.length, 0);
  const totalExamples = Array.from(examplesByTopic.values()).reduce((s, es) => s + es.length, 0);
  ...
  return {
    ...
    averageHealth: topicHealths.length > 0
      ? Math.round(topicHealths.reduce((s, t) => s + t.healthScore, 0) / topicHealths.length)
      : 0,
...
const isCLI = process.argv[1]?.endsWith("status.ts") || process.argv[1]?.endsWith("status.js");
if (isCLI) {
  ...
  const result = generateContentStatus(subject);
  if (!result) {
  ...
  if (jsonOutput) {
    ...
    console.log(`\n📊 Content Health: ${result.disciplineName} (${result.discipline})`);
    console.log(`${"=".repeat(90)}`);
    console.log(`Topics: ${result.topicCount} | Problems: ${result.totalProblems} | Examples: ${result.totalExamples}`);
    console.log(`Prerequisites: ${result.prerequisiteCount} | Encompassings: ${result.encompassingCount}`);
    console.log(`Simulation data: ${result.simDataAvailable ? "available" : "not found"} | Calibration issues: ${result.calibrationIssueCount}`);
    console.log();

    console.log(`Average health: ${result.averageHealth}/100`);
    const below50 = result.topics.filter(t => t.healthScore < 50);
    const below70 = result.topics.filter(t => t.healthScore < 70);
    ...
    const sorted = [...result.topics].sort((a, b) => a.healthScore - b.healthScore);
    for (const t of sorted) {
      ...
      console.log(
        `${marker}${t.topicId.padEnd(33)}${String(t.gradeLevel).padEnd(3)}${String(t.problemCount).padEnd(5)}${String(t.exampleCount).padEnd(4)}${diffStr.padEnd(12)}${t.demandDiversity.toString().padEnd(6)}${t.presentationCoverage.toString().padEnd(6)}${simStr.padEnd(6)}${String(t.healthScore).padEnd(5)}${issueStr}`
      );
    }
    ...
    const simTooHard = result.topics.filter(t => t.simStatus === "too-hard");
    if (simTooHard.length > 0) {
      console.log(`\n⚠️  Too hard for strong profiles (${simTooHard.length} topics):`);
      for (const t of simTooHard) {
        console.log(`  - ${t.topicId} (${t.simAccuracy !== null ? (t.simAccuracy * 100).toFixed(0) + "%" : "?"})`);
      }
    }

    const calTopics = result.topics.filter(t => t.difficultyCalibrationIssues.length > 0);
    if (calTopics.length > 0) {
      console.log(`\n⚠️  Difficulty calibration issues (${calTopics.reduce((s, t) => s + t.difficultyCalibrationIssues.length, 0)}):`);
      for (const t of calTopics) {
        console.log(`  ${t.topicId}: ${t.difficultyCalibrationIssues.join(", ")}`);
      }
    }
  }
}
